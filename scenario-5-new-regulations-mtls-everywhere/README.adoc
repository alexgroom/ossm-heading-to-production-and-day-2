...
----
./login-as.sh emma
oc -n prod-istio-system delete pods -l 'app in (istiod,istio-ingressgateway, istio-egressgateway,ge-ingressgateway)'
...
| ALLOW all | `prod-istio-system` -> `prod-travel-agency` | `ge-ingressgateway` | `travels.prod-travel-agency`, `flights.prod-travel-agency`, `hotels.prod-travel-agency`, `insurances.prod-travel-agency`, `cars.prod-travel-agency` | see SCRIPT below
...
-------------------------------------------------------------------
[ALLOW] ge-ingressgateway --> travels.prod-travel-agency
[ALLOW] ge-ingressgateway --> cars.prod-travel-agency
[ALLOW] ge-ingressgateway --> flights.prod-travel-agency
[ALLOW] ge-ingressgateway --> insurances.prod-travel-agency
[ALLOW] ge-ingressgateway --> hotels.prod-travel-agency
...
* In the previous link:https://gitlab.consulting.redhat.com/servicemesh-in-action/ossm-heading-to-production-and-day-2/-/blob/main/scenario-4-onboard-new-portal-with-authentication/README.adoc#user-content-create-requestauthentication-and-authorizationpolicy-to-allow-access-for-ge-user-gtouser[scenario-4] we applied an `AuthorizationPolicy` resource (`authpolicy-ge`) which allows requests flowing via the `ge-ingressgateway`. Now, we will utilize the `default-deny` pattern to DENY requests unless there is a specific `AuthorizationPolicy` allowing it.
...
| DENY all | `prod-istio-system` -> `prod-travel-agency` | `ge-ingressgateway` | `travels.prod-travel-agency`, `flights.prod-travel-agency`, `hotels.prod-travel-agency`, `insurances.prod-travel-agency`, `cars.prod-travel-agency` | see SCRIPT below
...
Authorization prod-istio-system --> prod-travel-agency
-------------------------------------------------------------------
[DENY] ge-ingressgateway --> travels.prod-travel-agency
[DENY] ge-ingressgateway --> cars.prod-travel-agency
[DENY] ge-ingressgateway --> flights.prod-travel-agency
[DENY] ge-ingressgateway --> insurances.prod-travel-agency
[DENY] ge-ingressgateway --> hotels.prod-travel-agency
...
2. As *farid* (`Mesh Operator`) _Travel Services Domain Owner (Tech Lead)_ allow access from `ge-ingressgateway` towards `travels.prod-travel-agency`, `hotels.prod-travel-agency`, `cars.prod-travel-agency`, `insurances.prod-travel-agency`, `flights.prod-travel-agency` in order to enable _GTO_ Travel search requests.
* At the `ge-ingressgateway` the _authz_ policy is set already to `ALLOW` calls on any /PATH for any external principal (link:https://istio.io/latest/docs/reference/config/security/authorization-policy/#Source[`requestPrincipals[*]`])
* The principal `"cluster.local/ns/prod-istio-system/sa/ge-ingressgateway-service-account"` will now be allowed to make calls to `prod-travel-agency` namespace.
+
----
./login-as.sh farid
oc apply -f authz-resources/03-ge-travels-to-travel-agency-allow.yaml
----
...
../scenario-4-onboard-new-portal-with-authentication/scripts/call-via-mtls-and-jwt-travel-agency-api.sh prod-istio-system ge $TOKEN
...
----
** Inspect KIALI and notice `ge-ingressgateway` -> `*.prod-travel-agecny` is fine but `flights` to `mysqldb` is not _authorized_. What can we do?
+
...
1. As *emma* (`Mesh Operator`) `DENY` communications from `ge-ingressgateway` towards any Operation that is link:https://istio.io/latest/docs/reference/config/security/authorization-policy/#Operation[`notPaths`] `/flights` or `/insurances`.
+
----
./login-as.sh emma
oc apply -f authz-resources/06-ge-travels-only-flights-insurances-paths-allow.yaml
----
