...
In this exercise we will showcase a process of tuning the `flights` and `mysqldb` _data plane_ components to cater for better performance when receiving upto *_250/rps_* from the external partner via `ge-ingressgateway` gateway. The link:https://linux.die.net/man/1/siege[siege] _http_ multi-threaded load testing and benchmarking utility has been employed to perform the tests.
...
siege -b -c500 -t60s https://ge-prod-istio-system.apps.ocp4.rhlab.de/flights/Tallinn --header="Authorization: Bearer $TOKEN"
----
...
+
image::./images/ge-ingressgateway-grafana-graph.png[300,700]
+
image::./images/ge-ingressgateway-kiali-metrics.png[300,700]
+
